...
def db():
    ...
    try:
        cursor.execute(
            'CREATE TABLE IF NOT EXISTS catalog (catalog_id integer NOT NULL PRIMARY KEY AUTOINCREMENT,name text not null,parent_catalog_id integer)')
        conn.commit()
        cursor.execute(
            'CREATE TABLE IF NOT EXISTS product (product_id integer not null PRIMARY KEY AUTOINCREMENT,catalog_id integer not null,name text not null,descriptions text,cost NUMERIC)')
        conn.commit()
        cursor.execute('CREATE TABLE IF NOT EXISTS address (link text,product_id integer,person_id integer, user_id_pony)')
        conn.commit()
